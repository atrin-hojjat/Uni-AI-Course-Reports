...
def run():
    ...
    for i in range(1, 8):
        classifier = DecisionTree(X_train, y_train, 'decision_tree_mushroom',
    ...
    plt.figure()
    plt.title("Decision Tree measures by tree depth")
    ...
    plt.legend()
    plot("DecisionTree measures by Max Depth")

    plt.figure()
    plt.title("Decision Tree running time by tree depth")
    colors = ["#112233", "#66CC99", "#44BBFF", "#FC575E", "#c955f7"]
    ...
    plot("Decision Tree running time by Max Depth")
